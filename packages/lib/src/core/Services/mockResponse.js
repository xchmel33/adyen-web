...
export async function mockResponse({
  ...
  if (mocks && uniqueFilename in mocks) {
    ...
    if (!result || result.status === 404) {
      console.warn(
        `Error getting unique mock response ${uniqueCacheId}. Will call backend server & save new.`,
      )
  ...
  // check if classic response exists
    ...
    if (mockConfig.recording && !mocks[filename].tests.includes(testName)) {
      ...
      if (
        !uniqueResponseBlacklist.includes(endpoint) &&
        !deepEqual(result, JSON.parse(JSON.stringify(response)))
      ) {
        await automockApp({
